...
for DOCKER_IMAGE in ${DOCKER_IMAGES}; do
  ...
  if [ "${DOCKER_REPOSITORY}" == "zammad-docker-compose" ]; then
    ...
    docker buildx build --platform linux/arm64v8 --pull --no-cache --build-arg BUILD_DATE="$(date -u +'%Y-%m-%dT%H:%M:%SZ')" -t "${DOCKER_REGISTRY}/${REPO_USER}/${DOCKER_REPOSITORY}:latest" -t "${DOCKER_REGISTRY}/${REPO_USER}/${DOCKER_REPOSITORY}:${ZAMMAD_VERSION}" .
